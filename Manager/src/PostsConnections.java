...
public class PostsConnections {
    ...
    public void addHistoryByPort(int port, String message) {
        Optional<PostsHistory> temp = historyList.stream().filter(s -> s.getPort() == port).findFirst();
        if (temp.isPresent())
    ...
    public void updateLastUsedService(int serviceInstance) {
        Optional<PostsHistory> temp = historyList.stream().filter(s -> s.getServiceInstance() == serviceInstance)
                .findFirst();
    ...
    public int getPort() {
        ...
        if (temp.isPresent()) {
            Optional<PostsHistory> foo = historyList
                    .stream()
                    .filter(
    ...
    public void setConnectedByPort(int port, boolean isConnected) {
        Optional<PostsHistory> temp = historyList.stream().filter(h -> h.getPort() == port).findFirst();
        if (temp.isPresent()) {
